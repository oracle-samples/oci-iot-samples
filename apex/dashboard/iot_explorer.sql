  ...
  from    (
              ...
              from    (
                        select  data_type,
                                  ...
                                  null as last_date
                          from    ( select 'Instances' as data_type, to_timestamp(i.data.timeCreated) as timecreated from instances_syn i where i.data."lifecycleState" = 'ACTIVE' 
                                    union all
                                    select 'Adaptors' as data_type, to_timestamp(ads.data.timeCreated) as timecreated from adapters_syn ads where ads.data."lifecycleState" = 'ACTIVE'
                                    union all
                                    select 'Models' as data_type, to_timestamp(ms.data.timeCreated) as timecreated from models_syn ms where ms.data."lifecycleState" = 'ACTIVE'
                                  )
                        )
              where   row_id = 1
          )
