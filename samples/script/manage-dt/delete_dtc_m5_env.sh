#!/usr/bin/env bash
#
# Delete Environmental digital twin (Custom format)
#
...
echo "${PGM}: Retrieve ${DTC_ENV_ID} ocid"
...
if [[ ! ${dt_id} =~ ^ocid1\.iotdigitaltwininstance\. ]]; then
  ...
  echo "${PGM}: Delete Environmental DT ${DTC_ENV_ID}"
...
if [[ ! ${adapter_id} =~ ^ocid1\.iotdigitaltwinadapter\. ]]; then
  ...
  echo "${PGM}: Delete adapter ${DTC_ENV_ADAPTER}"
  oci iot digital-twin-adapter delete --digital-twin-adapter-id "${adapter_id}"
fi

echo "${PGM}: Retrieve model ${DTC_ENV_MODEL_ID} ocid"
model_id=$(oci iot digital-twin-model list \
  --iot-domain-id "${IOT_DOMAIN_ID}" \
  --spec-uri-starts-with "${DTC_ENV_MODEL_ID}" \
  --lifecycle-state ACTIVE \
  --query "data.items[0].id" --raw-output
)
if [[ ! ${model_id} =~ ^ocid1\.iotdigitaltwinmodel\. ]]; then
  ...
  echo "${PGM}: Delete model ${DTC_ENV_MODEL_ID}"
