...
begin
  archive_domain_pkg.run(
    ...
    p_dataset_list => 'historized',
    p_start_time   => systimestamp - numtodsinterval(5, 'MINUTE'),
    p_end_time     => systimestamp,
    p_result       => l_result
  );

  select json_value(l_result, '$.datasets.historized.status' returning varchar2(32))
    into l_status
    from dual;

  if l_status is null or l_status != 'succeeded' then
    raise_application_error(-20010, 'historized export did not succeed');
  end if;
  ...
  select json_value(l_result, '$.datasets.historized.export_format' returning varchar2(32)),
         json_value(l_result, '$.datasets.historized.file_uri_list' returning varchar2(4000))
    into l_export_format, l_file_uri_list
    from dual;

  if l_export_format is null or l_export_format != 'parquet' then
    raise_application_error(-20016, 'historized export_format was not parquet');
  end if;

  if l_file_uri_list is null or regexp_like(l_file_uri_list, '\.dmp$') then
    raise_application_error(-20017, 'historized file_uri_list unexpectedly pointed to a dmp object');
  end if;
