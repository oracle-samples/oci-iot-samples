--
-- Smoke test for archive_domain_pkg.run on a small rejected window.
--
...
begin
  archive_domain_pkg.run(
    ...
    p_dataset_list => 'rejected',
    p_start_time   => systimestamp - numtodsinterval(5, 'MINUTE'),
    p_end_time     => systimestamp,
    p_result       => l_result
  );

  select json_value(l_result, '$.datasets.rejected.status' returning varchar2(32))
    into l_status
    from dual;

  if l_status is null or l_status != 'succeeded' then
    raise_application_error(-20010, 'rejected export did not succeed');
  end if;
  ...
  if l_export_format is null or l_export_format != 'parquet' then
    raise_application_error(-20016, 'rejected export_format was not parquet');
  end if;

  if l_file_uri_list is null or regexp_like(l_file_uri_list, '\.dmp$') then
    raise_application_error(-20017, 'rejected file_uri_list unexpectedly pointed to a dmp object');
  end if;
